...
Sequence& sort::BubbleSort(Sequence& ds) {
  bool sorted = false;
  while (!sorted) {
    sorted = true;
    for (int i = 0; i < ds.size - 1; i++) {
      if (ds[i] > ds[i+1]) {
        Sequence::Swap(ds[i], ds[i+1]);
        sorted = false;
      }
    }
  }
...
Sequence& sort::InsertionSort(Sequence& ds) {
...
Sequence& sort::SelectionSort(Sequence& ds) {
...
void Merge(Sequence& ds, int left, int mid, int right) {
...
void MergeSort(Sequence& ds, int left, int right) {
  if (left < right) {
...
Sequence& sort::MergeSort(Sequence& ds) {
...
void QuickSort(Sequence& ds, int left, int right) {
  if (left < right) {
    ...
    for (int j = left; j < right; j++) {
      if (ds[j] <= ds[pivot]) {
        i++;
        Sequence::Swap(ds[i], ds[j]);
      }
    }

    // Swap ds[i+1] with pivot
    Sequence::Swap(ds[i+1], ds[pivot]);
    pivot = i + 1;
...
Sequence& sort::QuickSort(Sequence& ds) {
